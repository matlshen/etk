...
class recursive_mutex {
  ...
  public:
    recursive_mutex() noexcept { ASSERT_0(__etk_recursive_mutex_init(&__m_)); }
    ...
    void lock() { ASSERT_0(__etk_recursive_mutex_lock(&__m_)); }

    bool try_lock() { return __etk_recursive_mutex_trylock(&__m_); }

    void unlock() { ASSERT_0(__etk_recursive_mutex_unlock(&__m_)); }
