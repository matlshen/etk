...
template <size_t N> class periodic_svc {
  public:
    periodic_svc();
    // periodic_svc(thread_attributes &attr);
    periodic_task_handle add_task(const functor<void()> &f,
                                  unsigned int period);
...
template <size_t N>
periodic_task_handle periodic_svc<N>::add_task(const functor<void()> &f, unsigned int period) {
...
template <size_t N>
periodic_task_handle periodic_svc<N>::add_task(void (*f)(), unsigned int period) {
    functor<void()> fc(*f);
    return add_task(fc, period);
}

template <size_t N> void periodic_svc<N>::join() { __t_.join(); }

template <size_t N> void periodic_svc<N>::start() {
    unique_lock __lk(__m_);
    stop_ = false;
    __cv_.notify_one();
}

template <size_t N> void periodic_svc<N>::stop() { stop_ = true; }

template <size_t N> void periodic_svc<N>::__svc(void *arg) {
    (void)arg;
    while (!stop_) {
        unique_lock __lk(__m_);
