...
#define ABORT_MSG(msg)                                                         \
    __etk_conditional_abort(                                                   \
        etk::__Assert::api_assert(false, msg, __FILE__, TO_STRING(__LINE__)))
#endif
...
#define ASSERT_UNREACHABLE()                                                   \
    etk::__Assert::api_assert(false, "Unreachable code", __FILE__,             \
                              TO_STRING(__LINE__))
#endif
#ifndef ASSERT_UNREACHABLE_FAIL
#define ASSERT_UNREACHABLE_FAIL()                                              \
    etk::__Assert::api_assert_fail(false, "Unreachable code", __FILE__,        \
                                   TO_STRING(__LINE__))
#endif
...
#define ASSERT_0(expr)                                                         \
    ...
    etk::__Assert::api_expect(__ASSERT_VAL == 0, "== ", __ASSERT_VAL, 0,       \
...
#define ASSERT_0_FAIL(expr)                                                    \
    __ASSERT_EVAL(0, expr);                                                    \
    etk::__Assert::api_expect_fail(__ASSERT_VAL == 0, "== ", __ASSERT_VAL, 0,  \
                                   __FILE__, TO_STRING(__LINE__));             \
    __ASSERT_EVAL_END
#endif
...
#define ASSERT_EQ(expected, expr)                                              \
    ...
    etk::__Assert::api_expect(__ASSERT_VAL == __ASSERT_EXPECTED,               \
...
#define ASSERT_EQ_FAIL(expected, expr)                                         \
    __ASSERT_EVAL(expected, expr);                                             \
    etk::__Assert::api_expect_fail(__ASSERT_VAL == __ASSERT_EXPECTED,          \
                                   "== ", __ASSERT_VAL, __ASSERT_EXPECTED,     \
                                   __FILE__, TO_STRING(__LINE__));             \
    __ASSERT_EVAL_END
