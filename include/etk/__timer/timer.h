...
#if defined(_ETK_HAS_NATIVE_TIMER)
#    include "etk/__timer/support.h"

// template <size_t StackSize>
// class timer {
//   private:
//     using TimerTaskCb = std::tuple<void (*)(void*), void*>;

//   public:
//     timer() = default;
//     ~timer() = default;

//     void add_task(void (*func)(void*), void* args, uint32_t delay, uint32_t
//     period) {
//         TimerTaskCb task = std::make_tuple(func, args);
//         ASSERT_0(__etk_timer_add_task(std::get<0>(task), std::get<1>(task),
//         delay, period));
//     }
// };
...
template <size_t StackSize, size_t N>
class timer {
  private:
    mutex __mutex_;
  ...
  public:
    ...
    void
    add_task(void (*func)(void*), void* args, uint32_t delay, uint32_t period) {
    ...
    void svc() {
        ...
        while (1) {
            ...
            // Wrap-around
                ...
                lock.unlock();
                this_thread::msleep(
                    UINT32_MAX - now + currentTask._next_expiry);
                lock.lock();
            }
