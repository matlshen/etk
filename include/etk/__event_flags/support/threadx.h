...
inline _ETK_INTERNAL_API int event_flags_set(__etk_event_flags_t *flags, uint32_t set) {
    return tx_event_flags_set(flags, set, TX_OR);
}

inline _ETK_INTERNAL_API int event_flags_clear(__etk_event_flags_t *flags, uint32_t clear) {
    return tx_event_flags_set(flags, ~clear, TX_AND);
}

inline _ETK_INTERNAL_API int event_flags_get(__etk_event_flags_t *flags, uint32_t *set, event_flags::mode mode) {
    ULONG flags_set;
