...
struct __Assert {
    ...
    static _ETK_API_INTERNAL void api_expect(long expr, const char *op,
    ...
    static _ETK_API_INTERNAL void api_assert_fail(int expr, const char *msg,
                                                  const char *file,
                                                  const char *line) {
        __etk_conditional_abort(__etk_assert(expr, msg, file, line));
    }

    static _ETK_API_INTERNAL void api_expect_fail(long expr, const char *op,
                                                  long val, long expected,
                                                  const char *file,
                                                  const char *line) {
